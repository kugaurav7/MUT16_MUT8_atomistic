...
def Hbond_interaction_profile( donor, hydrogen, acceptor, cutoff_distance=3.5, angle_cutoff=120):
    ...
    angle = np.arccos(np.dot(hydrogen -donor, hydrogen - acceptor) / (np.linalg.norm(hydrogen - donor) * np.linalg.norm(hydrogen - acceptor))) * 180 / np.pi
...
ser_ser_hbond = 0
total_ser_ser_hbond = len(ser_oxygen) * (len(ser_oxygen) - 1) / 2
...
thr_thr_hbond = 0
total_thr_thr_hbond = len(thr_oxygen) * (len(thr_oxygen) - 1) / 2
...
asn_asn_hbond = 0
total_asn_asn_hbond = len(asn_oxygen) * (len(asn_oxygen) - 1) / 2
...
gln_gln_hbond = 0
total_gln_gln_hbond = len(gln_oxygen) * (len(gln_oxygen) - 1) / 2
...
tyr_tyr_hbond = 0
total_tyr_tyr_hbond = len(tyr_oxygen) * (len(tyr_oxygen) - 1) / 2
...
# Plot the interaction profiles

plt.bar(range(13), interaction_profiles)
plt.xticks(range(13), ["SS", "TT", "NN", "QQ", "YY", "SY", "TY", "NY", "QY", "SN", "SQ", "TN", "TQ"])
plt.ylabel("Interaction probability (P)")
