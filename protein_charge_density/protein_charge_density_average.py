    ...
    for run in range(40):
        for clone in range(100):
            for result in range(100):
                density_profile_path = os.path.join(base_dir, f"RUN{run}", f"CLONE{clone}", f"results{result}", 'protein_charge_count.txt')
...
output_file = "/mnt/home/gkumar/Mut16_Mut8_atomistic_project_analysis/density_profile_average/protein_charge_density_avg_sem.txt"
